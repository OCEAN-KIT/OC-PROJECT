...
export default function EnvironmentLogList({
  ...
  const setField = <TKey extends keyof EnvironmentLogPayload>(
    key: TKey,
    value: EnvironmentLogPayload[TKey],
              ...
              {entries.map((entry) => (
                <tr key={entry.id} className="border-b last:border-0">
                  ...
                  <td className="px-4 py-3">
                    {entry.visibility && conditionBadge(entry.visibility)}
                  </td>
                  <td className="px-4 py-3">
                    {entry.current && conditionBadge(entry.current)}
                  </td>
                  <td className="px-4 py-3">
                    {entry.surge && conditionBadge(entry.surge)}
                  </td>
                  <td className="px-4 py-3">
                    {entry.wave && conditionBadge(entry.wave)}
                  </td>
                  <td className="px-4 py-3">
                    <button
                      type="button"
